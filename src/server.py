...
try:
    ...
    logging.info(
        f"Successfully initialized AIplatform credentials for project {aiplatform_project_id} in {aiplatform_location}"
    )
...
def convert_anthropic_to_litellm(anthropic_request: MessagesRequest) -> Dict[str, Any]:
    ...
    if (
        ...
        or anthropic_request.model.startswith("aiplatform/")
    ):
        max_tokens = min(max_tokens, 16384)
        logger.debug(
            f"Capping max_tokens to 16384 for OpenAI/Gemini/AIplatform model (original value: {anthropic_request.max_tokens})"
        )
                ...
                if request_data.get("stream", False):
                    ...
                    async def generate_responses():
                        ...
                        try:
                            ...
                            yield {
                                "choices": [
                                    {"delta": {"anthropic_event": "message_stop"}, "index": 0, "finish_reason": "stop"}
                                ],
                                "created": int(time.time()),
                                "model": clean_model_name,
                                "object": "chat.completion.chunk",
                            ...
                            yield {
                                "choices": [
                                    {"delta": {"content": f"Error: {str(e)}"}, "index": 0, "finish_reason": "stop"}
                                ],
                                "created": int(time.time()),
                                "model": clean_model_name,
                                "object": "chat.completion.chunk",
...
@app.get("/")
async def root():
    return {"message": "Anthropic Proxy for Thomson Reuters AIplatform"}
